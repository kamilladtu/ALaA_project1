...
def plot_mnist_optimization_path(opt):
    """Plots 3 maps of hyperparameters vs accuracy with the optimization path [1, 3]."""
    # 1. Extract data from the optimization result
    iters = np.array(opt.x_iters)
    accs = -opt.func_vals  # Convert back to accuracy (skopt minimizes) [3, 4]
    names = ['Batch Size', 'Learning Rate', 'Dropout Rate']
    pairs = [(0, 1), (0, 2), (1, 2)]
    gp_model = opt.models[-1] # The latest surrogate GP model [5, 6]
    ...
    for i, (idx1, idx2) in enumerate(pairs):
        ...
        # 2. Create a grid for the heatmap surrogate prediction [5, 7]
        d1_range = np.linspace(opt.space[idx1].low, opt.space[idx1].high, 50)
        d2_range = np.linspace(opt.space[idx2].low, opt.space[idx2].high, 50)
        X1, X2 = np.meshgrid(d1_range, d2_range)
        
        # 3. Prepare points for prediction (fix 3rd hyperparameter at best value) [2]
        grid_points = []
        for x1, x2 in zip(X1.ravel(), X2.ravel()):
            point = list(opt.x) # Use best parameters as the baseline
            point[idx1], point[idx2] = x1, x2
            grid_points.append(point)
            
        # Normalize inputs for the GP and predict accuracy [2, 5]
        grid_norm = opt.space.transform(grid_points)
        y_pred, _ = gp_model.predict(grid_norm)
        Z = (-y_pred).reshape(X1.shape) # Flip sign back to accuracy [5]

        # 4. Plot the Predicted Accuracy Heatmap [8]
        im = ax.contourf(X1, X2, Z, levels=25, cmap='viridis')
        fig.colorbar(im, ax=ax, label='Predicted Accuracy')
        
        # 5. Plot the Path and Iteration Points [1, 9, 10]
        ax.plot(iters[:, idx1], iters[:, idx2], 'r:', alpha=0.6, zorder=5) # Path line
        ax.scatter(iters[:, idx1], iters[:, idx2], c='white', edgecolors='black', zorder=10)
        
        # Add labels to points to show the sequence (1, 2, 3...) [1, 9]
        for j, (px, py) in enumerate(zip(iters[:, idx1], iters[:, idx2])):
            ax.text(px, py, str(j+1), color='red', fontsize=8, fontweight='bold', zorder=11)

        ax.set_xlabel(names[idx1])
        ax.set_ylabel(names[idx2])
        
        # Use log scale for Learning Rate as defined in search space [11]
        if 'Learning Rate' in [names[idx1], names[idx2]]:
            ax.set_xscale('log') if idx1 == 1 else ax.set_yscale('log')

    plt.tight_layout()
    plt.show()

# Usage: plot_mnist_optimization_path(opt)